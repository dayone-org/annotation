...
export function CommentMarkers() {
  const { activeThreadId, comments, currentPath, openThreadComposer, showResolved, updateThreadRect } = useAnnotation()
  ...
  const syncMarkerPositions = useCallback((syncState: boolean) => {
    ...
    for (const comment of topLevelCommentsRef.current) {
      ...
      const nextPosition = getMarkerPagePosition(comment.rect, querySelectorSafely(comment.selector))
  ...
  const finishDrag = useCallback(
    async (event: ReactPointerEvent<HTMLButtonElement>, session: DragSession) => {
      ...
      const nextPosition: MarkerPosition = {
        ...currentPosition,
        x: event.clientX + window.scrollX - session.pointerOffsetX - MARKER_OFFSET_X - session.stackedOffset.x,
        y: event.clientY + window.scrollY - session.pointerOffsetY - MARKER_OFFSET_Y - session.stackedOffset.y,
      }
      ...
      await updateThreadRect(
        session.threadId,
        measurePoint(nextPosition.x - window.scrollX, nextPosition.y - window.scrollY, session.target ?? undefined),
      )
    },
    [updateThreadRect],
  )
  ...
  const handlePointerCancel = useCallback(
    (event: ReactPointerEvent<HTMLButtonElement>) => {
      ...
      if (!threadId || !session || session.threadId !== threadId || session.pointerId !== event.pointerId) {
        return
      }
  ...
  useEffect(() => {
    ...
    topLevelCommentMapRef.current = new Map(topLevelComments.map((comment) => [comment.id, comment]))
  ...
  return (
    <div className="pointer-events-none absolute inset-0 z-2147483601">
      {topLevelComments.map((comment) => {
        ...
        return (
          <button
            key={comment.id}
            className={cn(
              'pointer-events-auto absolute left-0 top-0 inline-flex size-8 items-center justify-center rounded-full shadow-lg cursor-grab active:cursor-grabbing select-none touch-none text-xs',
            ...
            type="button"
          >
            <Badge className="absolute right-0 top-0 size-5 translate-x-1/2 -translate-y-1/2 bg-primary text-xs tabular-nums text-primary-foreground">
              {threadCount}
            </Badge>
